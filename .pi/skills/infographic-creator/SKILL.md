...
description: Create infographics from Agrotrace data and operational summaries. Use when users ask for infografico, resumo visual, visual storytelling, or infographic exports.
---
...
## Agrotrace Project Workflow (required)

For this project, do not create infographics from guesswork.

1. Query real data first using Agrotrace tools (`agrotrace_kpi_snapshot`, `agrotrace_get_*_context`).
2. Confirm scope and filters used (`certificadoraId`, `usuarioId`, optional `safraId`/`propriedadeId`).
3. Build the AntV syntax from the retrieved data and save the syntax in `./.output/reports/<slug>-infographic.antv.txt`.
4. Generate an HTML viewer in `./.output/reports/<slug>-infographic.html`.
5. Return to the user:
   ...
   - syntax used
   - short explanation of what the infographic highlights

If the user only wants a chart (not a narrative infographic), prefer `chart-visualization` skill.
...
For Agrotrace tasks, perform data retrieval first and only then structure the infographic.
...
- Use only retrieved values and mention filters in the description when relevant.
...
2. Use the Write tool to generate HTML file, named as `./.output/reports/<slug>-infographic.html`
